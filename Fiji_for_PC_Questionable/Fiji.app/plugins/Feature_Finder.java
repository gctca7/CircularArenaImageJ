...
public class Feature_Finder implements ExtendedPlugInFilter, DialogListener, ActionListener {
    ...
    public final static int OUTTYPE_POINT_IMAGE = 3;
    /** Output type: deviation map */
    public final static int OUTTYPE_MAP = 4;
    /** Output type: average of all features found */
    public final static int OUTTYPE_AVERAGE = 5;
    //
    private final static String[] outTypes = new String[]
            {"Point Selection", "Count", "List", "Point Map", "Deviation Map", "Average of Features"};
    //filter parameters
    ...
    public void run (ImageProcessor ip) {
        ...
        if (outType==OUTTYPE_COUNT) {
            ...
            rt.show("Results");
        } else if (outType==OUTTYPE_LIST) {
            Analyzer.resetCounter();
            ResultsTable rt = ResultsTable.getResultsTable();
            for (int i=0, n=0; i<pointList[2].length; i++) if (pointOKlist[i]) {
                if (pointList[2][i]<(0.01*tolerance+1e-6)) {
                    rt.incrementCounter();
        			rt.addValue("X", pointList[0][i]);
        			rt.addValue("Y", pointList[1][i]);
                }
            }
            rt.show("Results");
